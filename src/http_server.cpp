...
constexpr char kIndexHtml[] =
    ...
    "<main>"
    "<h1>モバイルキーボード</h1>"
    "<form id=\"textForm\" method=\"post\" action=\"/text\">"
    "<textarea id=\"text\" name=\"text\" autofocus></textarea>"
    "<div class=\"row\">"
    ...
    "<div class=\"row\">"
    "<input id=\"shortcut\" name=\"key\" placeholder=\"CTRL+SHIFT+ESC\">"
    "<button class=\"primary\" type=\"submit\">ショートカット送信</button>"
    "</div>"
    "</form>"
    "</main>"
    "<script>"
    "function enc(o){return new URLSearchParams(o).toString()}"
    "function post(p,o){return fetch(p,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:enc(o||{}),cache:'no-store',redirect:'follow'})}"
    "addEventListener('DOMContentLoaded',function(){"
    "var t=document.getElementById('text');"
    "document.getElementById('textForm').addEventListener('submit',function(e){e.preventDefault();post('/text',{text:t.value}).then(function(){t.focus()})});"
    "document.querySelectorAll('button[formaction]').forEach(function(b){b.addEventListener('click',function(e){e.preventDefault();post(b.getAttribute('formaction'),{}).then(function(){t.focus()})})});"
    "document.querySelector('form[action=\"/shortcut\"]').addEventListener('submit',function(e){e.preventDefault();var k=document.getElementById('shortcut');post('/shortcut',{key:k.value}).then(function(){k.focus()})});"
    "});"
    "</script>"
    "</body>"
    "</html>";
